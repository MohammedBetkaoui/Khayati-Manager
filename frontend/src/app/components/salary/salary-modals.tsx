...
export function CalculateSalaryModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const helperText = 
    type === "daily" ? t.methodDaily :
    type === "piece" ? t.methodPiece :
    type === "mixed" ? t.methodMixed :
    lang === "ar" ? "إدخال الراتب الثابت المتفق عليه" : "Saisir le salaire fixe convenu";
    ...
    <ModalShell open={open} onClose={onClose} title={t.title} maxWidth={540}>
      <div className="grid grid-cols-1 gap-5 px-6 py-5 sm:grid-cols-2">
        ...
        <Field label={t.period}>
          <TextInput placeholder={lang === "ar" ? "مثال: جوان 2026" : "Ex: Juin 2026"} />
        </Field>
        ...
        {(type === "daily" || type === "weekly" || type === "monthly" || type === "mixed") && (
          <Field label={t.base}>
            <TextInput type="number" value={base} onChange={e => setBase(e.target.value)} placeholder="0" />
          </Field>
        )}
        ...
        {(type === "piece" || type === "mixed") && (
          <>
            <Field label={t.piecesCount}>
              <TextInput type="number" value={pieces} onChange={e => setPieces(e.target.value)} placeholder="0" />
            </Field>
            <Field label={t.pieceRate}>
              <TextInput type="number" value={rate} onChange={e => setRate(e.target.value)} placeholder="0" />
            </Field>
          </>
        )}
        ...
        <div className="sm:col-span-2 mt-2 rounded-xl bg-black/5 p-4 text-sm">
          <div className="text-muted-foreground font-semibold mb-2">{t.methodTitle}</div>
          <div className="mb-3" style={{ fontSize: 12.5, color: palette.muted }}>{helperText}</div>
          
          <div className="flex justify-between border-t pt-2 font-bold" style={{ borderColor: palette.borderStrong, color: palette.primary, fontSize: 15 }}>
            <span>{t.net}:</span>
            <span>{calculatedBase.toLocaleString()} {cur}</span>
          </div>
        </div>
...
export function AdvanceModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { lang } = useLanguage();
  ...
  return (
    <ModalShell open={open} onClose={onClose} title={t.title} maxWidth={440}>
      <div className="flex flex-col gap-4 px-6 py-5">
        <Field label={t.worker}>
          <TextInput placeholder={lang === "ar" ? "اختيار العامل..." : "Choisir travailleur..."} />
        </Field>
        ...
        <Field label={t.deductMethod}>
          <select
            className="w-full"
            style={{ height: 42, padding: "0 14px", borderRadius: 12, border: `1px solid ${palette.border}`, outline: "none" }}
          >
...
export function BonusDeductionModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { lang } = useLanguage();
...
export function PaymentModal({ open, onClose, record }: { open: boolean; onClose: () => void; record: PayrollRecord | null }) {
  const { lang } = useLanguage();
    ...
    <ModalShell open={open} onClose={onClose} title={t.title} maxWidth={440}>
      <div className="px-6 py-5">
        {record && (
          <div className="mb-5 rounded-xl border p-4" style={{ borderColor: palette.border, backgroundColor: palette.bg }}>
            <div className="flex justify-between items-center mb-1">
              <span style={{ fontSize: 13, color: palette.muted }}>{t.worker}</span>
              <span style={{ fontSize: 14, fontWeight: 700 }}>{record.workerName[lang]}</span>
            </div>
            <div className="flex justify-between items-center">
              <span style={{ fontSize: 13, color: palette.muted }}>{t.net}</span>
              <span style={{ fontSize: 15, fontWeight: 800, color: palette.primary }}>{net.toLocaleString()} {cur}</span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-4">
          <Field label={t.paid}>
            <TextInput type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder={net.toString()} />
          </Field>
          ...
          <div className="mt-2 rounded-xl bg-black/5 p-4 text-sm">
            <div className="flex justify-between border-t pt-2 font-bold" style={{ borderColor: palette.borderStrong, color: rem > 0 ? "#b46a66" : "#4d8a6a" }}>
              <span>{t.remaining}:</span>
              <span>{rem.toLocaleString()} {cur}</span>
            </div>
          </div>
